/******************************************************************************
	...
	Synopsis: This file contains function definitions for the Object class.
******************************************************************************/
...
void Object::getObjInfo()
{
    ...
    if (infile) {
        ...
        while (!infile.eof()) {
            ...
            if (read == "v") {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    // the vertIndex array
                    vertexIndex.push_back(int(digit - 1));
                    ...
                    digit = 0;	p++;
                    while (isdigit(thisCorner[p])) digit = digit * 10 + thisCorner[p++] - 48;
                    normalsIndex.push_back(int(digit - 1));
                }
            } // end faces
            infile >> read;
        ...
        std::cout << "Verticies: " << numVert << std::endl;
        std::cout << "Normals: " << numNorm << std::endl;
        std::cout << "Indicies: " << numIndex << std::endl;
    } // end if(infile)
    else {
...
int Object::initVertArray(GLfloat **vertexArrayPointer) {
    GLfloat *vertexArray = (GLfloat *) malloc(sizeof(GLfloat[3]) * numVert);
    for (int i = 0, j = 0; i < vertexIndex.size(); i++, j += 3) {
        vertexArray[j] = tempVertList[vertexIndex[i]][0];
        vertexArray[j + 1] = tempVertList[vertexIndex[i]][1];
        vertexArray[j + 2] = tempVertList[vertexIndex[i]][2];
    }
    *vertexArrayPointer = vertexArray;
    return numVert;
}
